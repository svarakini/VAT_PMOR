...
%          ActiveDof=setdiff(FEM.GDof/5*3+1:FEM.GDof,Constrained_Dof);
    ...
    case 'SSSS-NXX-Wu-middle'
        
        
        % middle nodes
        % middle nodes = x/2 fixed in u, v are free
        SideNodesNumList_MID_Xhalf = find(xx == max(FEM.nodeCoordinates(:,1))/2)';
        
        
%         figure(200);hold on; plot(xx( SideNodesNumList_MID_Xhalf ),yy( SideNodesNumList_MID_Xhalf ),'MarkerFaceColor',[0 0 1],'MarkerEdgeColor',[0 0 1],'Marker','o',...
%             'LineStyle','none',...
%             'Color',[0 0 1]);
       % 
       % middle x=a/2, u=w=0
        SideNodesNum_MIDX=length(SideNodesNumList_MID_Xhalf);
        SideNodesNum=SideNodesNum_MIDX;
        SideNodesNumList=SideNodesNumList_MID_Xhalf ;
%         SidesDof_MIDX(1:SideNodesNum)=SideNodesNumList;%% w is constrainted
        SidesDof_MIDX(1*SideNodesNum+1:SideNodesNum*2)=SideNodesNumList+3*nodeNum; %% u is constrainted
...
%         SidesDof_BHS(1*SideNodesNum+1:SideNodesNum*2) = SideNodesNumList+4*nodeNum; %% v is constrainted
        ...
        % ActiveDof=setdiff(1:FEM.GDof/5*3,SidesDof);
        Constrained_Dof= unique([SidesDof_MIDX]); % SidesDof_LHS SidesDof_RHS SidesDof_THS SidesDof_BHS]);
        ActiveDof=setdiff(1:FEM.GDof,Constrained_Dof);
%         ActiveDof=setdiff(FEM.GDof/5*3+1:FEM.GDof,Constrained_Dof);
...
%         SidesDof_BHS(1:SideNodesNum)=SideNodesNumList;%% w is constrainted
        ...
        case 'SSSS-NXX-Wu-middle-Coburn'
        % may need no
        
        % middle nodes
        % top and bottom edges, v=0
        SideNodesNumList_MID_Xhalf = find(xx == max(FEM.nodeCoordinates(:,1))/2)';
...
%         SidesDof_BHS(1:SideNodesNum) = SideNodesNumList;%% w is constrainted
        ...
        % ActiveDof=setdiff(1:FEM.GDof/5*3,SidesDof);
        Constrained_Dof= unique([SidesDof_THS SidesDof_BHS]); % SidesDof_LHS SidesDof_RHS SidesDof_THS SidesDof_BHS]);
        ActiveDof=setdiff(1:FEM.GDof,Constrained_Dof);
%         ActiveDof=setdiff(FEM.GDof/5*3+1:FEM.GDof,Constrained_Dof);
